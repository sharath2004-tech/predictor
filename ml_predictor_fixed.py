    ...
    try:
        ...
        for window in [5, 10, 20, 50]:
            try:
                ...
                ma_nonzero = ma.replace(0, 0.001)  # Avoid division by zero
        ...
        for span in [12, 26]:
            try:
                ...
                ema_nonzero = ema.replace(0, 0.001)  # Avoid division by zero
        ...
        if len(volume_series) > 0:
            try:
                ...
                volume_ma_nonzero = volume_ma.replace(0, 1)  # Avoid division by zero
        ...
        try:
            ...
            for name, model in self.models.items():
                with st.spinner(f"Training {name}..."):
                    ...
                    # Store results as basic Python types
                    results[name] = {
                        'model': model,
    ...
    def predict_future(self, features_df, days_ahead=7):
        ...
        try:
            ...
            for name, model_data in self.trained_models.items():
                ...
                # Scale features
                X_latest = scaler.transform(latest_features[feature_columns].values)
                
                # Make prediction
                pred = model.predict(X_latest)[0]
                
                # For simplicity, predict same value for all days (can be improved)
                future_preds = [float(pred)] * days_ahead
...
def show_ml_predictions(data, stock_symbol, days_ahead=7, prediction_date=None, min_confidence=75):
    ...
    try:
        ...
        # Filter models by confidence threshold - ULTRA SAFE VERSION
        qualified_models = {}
        current_price = float(data['Close'].iloc[-1])
        
        for name, results in models_results.items():
            try:
                # Extract RMSE as basic float
                rmse_val = results['test_rmse']
                ...
                # Create completely new dictionary with guaranteed scalar values
                safe_result = {
                    'test_rmse': rmse_val,
                ...
                if 'feature_importance' in results:
                    safe_result['feature_importance'] = results['feature_importance']
                
                # Only add if meets confidence threshold
                if confidence_val >= min_confidence:
                    qualified_models[name] = safe_result
        ...
        if not qualified_models:
            ...
            # Add all models with safe processing
            for name, results in models_results.items():
                try:
                    rmse_val = float(results['test_rmse'])
                    confidence_val = max(0.0, min(100.0, 100.0 - (rmse_val / current_price * 100.0)))
                    
                    qualified_models[name] = {
                        'test_rmse': rmse_val,
                        'test_mae': float(results['test_mae']),
                        'test_r2': float(results['test_r2']),
                        'confidence': confidence_val,
                    ...
                    if 'feature_importance' in results:
                        qualified_models[name]['feature_importance'] = results['feature_importance']
        ...
        try:
            ...
            for name, results in qualified_models.items():
                try:
                    # All values are now guaranteed to be basic Python floats
                    rmse_val = results['test_rmse']
                    mae_val = results['test_mae']
                    r2_val = results['test_r2']
                    confidence_val = results['confidence']
                    
                    # Create pre-formatted strings to avoid f-string issues
        ...
        date_str = prediction_date.strftime('%B %d, %Y') if prediction_date else f'{days_ahead} Days Ahead'
        st.markdown(f"### 🔮 Price Prediction for {date_str}")
        ...
        if future_predictions:
            ...
            for i, (name, pred_data) in enumerate(future_predictions.items()):
                try:
                    ...
                    confidence_val = model_result.get('confidence', 0.0)
        ...
        best_rmse = best_results['test_rmse']
        best_r2 = best_results['test_r2']
        best_confidence = best_results['confidence']
        best_prediction = float(future_predictions[best_name]['predictions'][-1])
        
        prediction_target = prediction_date.strftime('%B %d, %Y') if prediction_date else f'{days_ahead} days'
        ...
        - **Prediction for {prediction_target}**: ₹{best_prediction:.2f}
        ...
        if ensemble_prediction and len(future_predictions) > 1:
            ...
            avg_confidence = best_results['confidence']
        ...
        target_display = prediction_date.strftime('%B %d, %Y') if prediction_date else f'{days_ahead} days from now'
